...
public class NonlinearFactorTest {
    ...
    void testNonlinearFactor() {
        ...
        // Vector actual_e =
        // std::dynamic_pointer_cast<NoiseModelFactor>(factor)->unwhitenedError(cfg);
    ...
    // static_assert(std::is_same<Base, NoiseModelFactor>::value, "Base type
    // wrong");
    ...
    // TestFactor4() : Base(noiseModel::Diagonal::Sigmas((Vector(1) <<
    // 2.0).finished()), X(1), X(2), X(3), X(4)) {}

    // // Provide access to the Matrix& version of evaluateError:
    // using Base::NoiseModelFactorT; // inherit constructors

    // Vector
    // evaluateError(const double& x1, const double& x2, const double& x3, const
    // double& x4,
    ...
    void testNoiseModelFactor4() throws Throwable {
        ...
        // JacobianFactor
        // jf(*std::dynamic_pointer_cast<JacobianFactor>(tf.linearize(tv)));
        ...
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 0.5).finished(),
        // jf.getA(jf.begin())));
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 1.0).finished(),
        // jf.getA(jf.begin()+1)));
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 1.5).finished(),
        // jf.getA(jf.begin()+2)));
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 2.0).finished(),
        // jf.getA(jf.begin()+3)));
        // EXPECT(assert_equal((Vector)(Vector(1) << 0.5 * -30.).finished(),
        // jf.getb()));
    ...
    // class TestFactor5 : public NoiseModelFactor5<double, double, double, double,
    // double> {
    // public:
    ...
    // TestFactor5() : Base(noiseModel::Diagonal::Sigmas((Vector(1) <<
    // 2.0).finished()), X(1), X(2), X(3), X(4), X(5)) {}

    // Vector
    // evaluateError(const X1& x1, const X2& x2, const X3& x3, const X4& x4, const
    // X5& x5,
    ...
    void testNoiseModelFactor5() throws Throwable {
        ...
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 2.0).finished(),
        // jf.getA(jf.begin()+3)));
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 2.5).finished(),
        // jf.getA(jf.begin()+4)));
        // EXPECT(assert_equal((Vector)(Vector(1) << 0.5 * -55.).finished(),
        // jf.getb()));
    }

    // class TestFactor6 : public NoiseModelFactor6<double, double, double, double,
    // double, double> {
    // public:
    // typedef NoiseModelFactor6<double, double, double, double, double, double>
    // Base;

    // // Provide access to the Matrix& version of evaluateError:
    // using Base::evaluateError;

    // TestFactor6() : Base(noiseModel::Diagonal::Sigmas((Vector(1) <<
    // 2.0).finished()), X(1), X(2), X(3), X(4), X(5), X(6)) {}
    ...
    void testNoiseModelFactor6() throws Throwable {
        ...
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 2.5).finished(),
        // jf.getA(jf.begin()+4)));
        // EXPECT(assert_equal((Matrix)(Matrix(1, 1) << 3.0).finished(),
        // jf.getA(jf.begin()+5)));
        // EXPECT(assert_equal((Vector)(Vector(1) << 0.5 * -91.).finished(),
        // jf.getb()));
    ...
    // TestFactorN() : Base(noiseModel::Diagonal::Sigmas((Vector(1) <<
    // 2.0).finished()), X(1), X(2), X(3), X(4)) {}
